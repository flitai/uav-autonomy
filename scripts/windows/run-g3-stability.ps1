...
param(
    ...
    [ValidateSet('','amase-early-exit','uxas-early-exit','planning-timeout','shutdown-timeout','disconnect-partial')][string]$TestFault='',
    [switch]$ChinesePath,
    [switch]$Verify
)
...
if(-not $RunId) { $RunId='g3-t06-'+$(if($Verify){'test'}else{'run'})+'-'+(Get-Date -Format 'yyyyMMdd-HHmmss-fff') }
if($RunId -notmatch '^g3-t06-[A-Za-z0-9-]{1,80}$') { throw 'Invalid G3 run identity.' }
...
$record=[ordered]@{task='G3-T06';runId=$RunId;status='running';startedAt=(Get-Date -Format o);invocationDirectory=$location.Path}
try {
    ...
    if(-not $Configuration) { $Configuration=Join-Path $root 'config/g3-stability.json' }
    ...
    Write-CppJson @{baselineRunId=$BaselineRunId;javaHome=$env:JAVA_HOME;configuration=(Join-Path $root 'config/g3-startup.json');executionConfiguration=(Join-Path $root 'config/g3-execution.json');stabilityConfiguration=$Configuration;completionRunId=$CompletionRunId;mode=$Mode;verify=[bool]$Verify} (Join-Path $run 'context.json')
    $arguments=@('-I','-B','-X','utf8',(Join-Path $root 'scripts/g3_stability/runtime.py'),'--root',$root,'--run-id',$RunId,'--mode',$Mode)
    if($Verify) { $arguments+='--verify' }
    if($CompletionRunId -notmatch '^g3-t05-[A-Za-z0-9-]+$') { throw 'Invalid completion identity.' }
    if($TestFault) { $arguments+=@('--fault',$TestFault) }
    if($ChinesePath) { $arguments+='--chinese-path' }
    & $PythonExecutable @arguments
    if($LASTEXITCODE -ne 0) { throw "G3 stability failed; see $run/result.json" }
    ...
    Write-Host "G3_T06_RUN_ID=$RunId"
