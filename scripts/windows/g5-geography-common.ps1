...
function Invoke-G5Geography {
    param([string]$Action, [string]$PythonExecutable, [string]$BuildRunId)
    $repoRoot = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot '../..'))
    $runId = 'g5-t02-' + $Action + '-' + (Get-Date -Format 'yyyyMMdd-HHmmss-fff')
    ...
    $record = [ordered]@{ task='G5-T02'; runId=$runId; action=$Action; status='running'; invokedFrom=$locationBefore.Path }
    try {
        ...
        $arguments = @('-I','-B','-X','utf8',(Join-Path $repoRoot 'scripts/g5_geography/manage.py'),
            '--run-id',$runId,'--baseline-run-id',$ids[0],'--action',$Action)
        if ($BuildRunId) { $arguments += @('--build-run-id',$BuildRunId) }
        & $PythonExecutable @arguments
        if ($LASTEXITCODE -ne 0) { throw 'G5 geography failed; inspect result and geography.stderr' }
        ...
        Write-Host ('G5_T02_RUN_ID=' + $runId)
    }
