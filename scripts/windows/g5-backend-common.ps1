...
function Invoke-G5Backend {
    param([string]$Action, [string]$PythonExecutable, [string]$BaselineRunId,
          [string]$BuildRunId, [string]$FlightRunId, [string]$Mode='Both')
    $repoRoot = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot '../..'))
    $runId = 'g5-t04-' + $Action + '-' + (Get-Date -Format 'yyyyMMdd-HHmmss-fff')
    ...
    $record = [ordered]@{ task='G5-T04'; action=$Action; runId=$runId; status='running'; invokedFrom=$locationBefore.Path }
    try {
        ...
        $arguments = @('-I','-B','-X','utf8',(Join-Path $repoRoot 'scripts/g5_backend/manage.py'),'--action',$Action,'--run-id',$runId,'--baseline-run-id',$BaselineRunId)
        if ($BuildRunId) { $arguments += @('--build-run-id',$BuildRunId) }
        if ($FlightRunId) { $arguments += @('--flight-run-id',$FlightRunId) }
        $arguments += '--modes'
        if ($Mode -eq 'Both') { $arguments += @('Headless','Gui') } else { $arguments += $Mode }
        & $PythonExecutable @arguments
        if ($LASTEXITCODE -ne 0) { throw 'G5 terrain backend failed; see result.json' }
        ...
        Write-Host ('G5_T04_RUN_ID=' + $runId)
    }
