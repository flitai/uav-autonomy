...
if(-not $RunId) { $RunId='g4-t04-'+$(if($Verify){'test'}else{'run'})+'-'+(Get-Date -Format 'yyyyMMdd-HHmmss-fff') }
if($RunId -notmatch '^g4-t04-[A-Za-z0-9-]{1,80}$') { throw 'Invalid G4 web run identity.' }
...
$record=[ordered]@{task='G4-T04';runId=$RunId;status='running';startedAt=(Get-Date -Format o);invocationDirectory=$location.Path}
try {
    ...
    $arguments=@('-I','-B','-X','utf8',(Join-Path $root 'scripts/g4_web/runtime.py'),'--root',$root,'--run-id',$RunId,'--mode',$Mode)
    if($Verify) { $arguments+='--verify' }
    & $PythonExecutable @arguments
    if($LASTEXITCODE -ne 0) { throw "G4 web failed; see $run/result.json" }
    ...
    Write-Host "G4_T04_RUN_ID=$RunId"
