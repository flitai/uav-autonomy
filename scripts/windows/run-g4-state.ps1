...
if(-not $RunId) { $RunId='g4-t03-'+$(if($Verify){'test'}else{'run'})+'-'+(Get-Date -Format 'yyyyMMdd-HHmmss-fff') }
if($RunId -notmatch '^g4-t03-[A-Za-z0-9-]{1,80}$') { throw 'Invalid G4 state run identity.' }
...
$record=[ordered]@{task='G4-T03';runId=$RunId;status='running';startedAt=(Get-Date -Format o);invocationDirectory=$location.Path}
try {
    ...
    Write-CppJson @{baselineRunId=$BaselineRunId;javaHome=$env:JAVA_HOME;configuration=$Configuration;mode=$Mode;keepGui=$false;verify=[bool]$Verify} (Join-Path $run 'context.json')
    $arguments=@('-I','-B','-X','utf8',(Join-Path $root 'scripts/g4_state/runtime.py'),'--root',$root,'--run-id',$RunId,'--mode',$Mode)
    if($Verify) { $arguments+='--verify' }
    & $PythonExecutable @arguments
    if($LASTEXITCODE -ne 0) { throw "G4 state failed; see $run/result.json" }
    ...
    Write-Host "G4_T03_RUN_ID=$RunId"
