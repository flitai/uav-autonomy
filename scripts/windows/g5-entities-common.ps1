...
function Invoke-G5Entities {
    ...
    $runId='g5-t06-'+$Action+'-'+(Get-Date -Format 'yyyyMMdd-HHmmss-fff')
    ...
    $record=[ordered]@{task='G5-T06';runId=$runId;action=$Action;status='running';invokedFrom=$locationBefore.Path}
    try {
        ...
        $arguments=@('-I','-B','-X','utf8',(Join-Path $root 'scripts/g5_entities/manage.py'),'--action',$Action,'--run-id',$runId,'--baseline-run-id',$BaselineRunId,'--mode',$Mode)
        if ($BuildRunId) {$arguments+=@('--build-run-id',$BuildRunId)}
        if ($ChinesePath) {$arguments+='--chinese-path'}
        & $PythonExecutable @arguments
        if ($LASTEXITCODE -ne 0) {throw 'G5-T06 failed; inspect result.json'}
        ...
        Write-Host ('G5_T06_RUN_ID='+$runId)
    }
