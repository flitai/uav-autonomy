...
if(-not $RunId) { $RunId='g3-t07-test-'+(Get-Date -Format 'yyyyMMdd-HHmmss-fff') }
if($RunId -notmatch '^g3-t07-[A-Za-z0-9-]{1,80}$') { throw 'Invalid G3 stage run identity.' }
if($StabilityRunId -notmatch '^g3-t06-[A-Za-z0-9-]+$') { throw 'Invalid stability identity.' }
...
$record=[ordered]@{task='G3-T07';runId=$RunId;status='running';startedAt=(Get-Date -Format o);invocationDirectory=$location.Path}
try {
    ...
    if(-not $Configuration) { $Configuration=Join-Path $root 'config/g3-acceptance.json' }
    ...
    Write-CppJson @{baselineRunId=$BaselineRunId;javaHome=$env:JAVA_HOME;configuration=(Join-Path $root 'config/g3-startup.json');executionConfiguration=(Join-Path $root 'config/g3-execution.json');completionConfiguration=(Join-Path $root 'config/g3-completion.json');acceptanceConfiguration=$Configuration;stabilityRunId=$StabilityRunId;mode='Headless';verify=$true} (Join-Path $run 'context.json')
    & $PythonExecutable -I -B -X utf8 (Join-Path $root 'scripts/g3_acceptance/runtime.py') --root $root --run-id $RunId
    if($LASTEXITCODE -ne 0) { throw "G3 stage acceptance failed; see $run/result.json" }
    ...
    Write-Host "G3_T07_RUN_ID=$RunId"
