...
if(-not $RunId) { $RunId='g4-t06-'+$(if($Verify){'test'}else{'run'})+'-'+(Get-Date -Format 'yyyyMMdd-HHmmss-fff') }
if($RunId -notmatch '^g4-t06-[A-Za-z0-9-]{1,80}$') { throw 'Invalid G3 run identity.' }
...
$record=[ordered]@{task='G4-T06';runId=$RunId;status='running';startedAt=(Get-Date -Format o);invocationDirectory=$location.Path}
try {
    ...
    if(-not $Configuration) { $Configuration=Join-Path $root 'config/g3-completion.json' }
    ...
    Write-CppJson @{baselineRunId=$BaselineRunId;javaHome=$env:JAVA_HOME;configuration=(Join-Path $root 'config/g3-startup.json');executionConfiguration=(Join-Path $root 'config/g3-execution.json');completionConfiguration=$Configuration;mode=$Mode;verify=[bool]$Verify} (Join-Path $run 'context.json')
    $arguments=@('-I','-B','-X','utf8',(Join-Path $root 'scripts/g4_completion/runtime.py'),'--root',$root,'--run-id',$RunId,'--mode',$Mode)
    if($Verify) { $arguments+='--verify' }
    & $PythonExecutable @arguments
    if($LASTEXITCODE -ne 0) { throw "G4 completion failed; see $run/result.json" }
    ...
    Write-Host "G4_T06_RUN_ID=$RunId"
