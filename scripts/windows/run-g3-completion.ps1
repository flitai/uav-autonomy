...
if(-not $RunId) { $RunId='g3-t05-'+$(if($Verify){'test'}else{'run'})+'-'+(Get-Date -Format 'yyyyMMdd-HHmmss-fff') }
if($RunId -notmatch '^g3-t05-[A-Za-z0-9-]{1,80}$') { throw 'Invalid G3 run identity.' }
...
$record=[ordered]@{task='G3-T05';runId=$RunId;status='running';startedAt=(Get-Date -Format o);invocationDirectory=$location.Path}
try {
    ...
    $arguments=@('-I','-B','-X','utf8',(Join-Path $root 'scripts/g3_completion/runtime.py'),'--root',$root,'--run-id',$RunId,'--mode',$Mode)
    if($Verify) { $arguments+='--verify' }
    & $PythonExecutable @arguments
    if($LASTEXITCODE -ne 0) { throw "G3 completion failed; see $run/result.json" }
    ...
    Write-Host "G3_T05_RUN_ID=$RunId"
