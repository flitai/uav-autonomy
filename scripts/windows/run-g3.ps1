...
if(-not $RunId) { $RunId='g3-t03-'+$(if($Verify){'test'}else{'run'})+'-'+(Get-Date -Format 'yyyyMMdd-HHmmss-fff') }
if($RunId -notmatch '^g3-t03-[A-Za-z0-9-]{1,80}$') { throw 'Invalid G3 run identity.' }
...
$record=[ordered]@{task='G3-T03';runId=$RunId;status='running';startedAt=(Get-Date -Format o);invocationDirectory=$location.Path}
try {
    ...
    if(-not $Configuration) { $Configuration=Join-Path $root 'config/g3-startup.json' }
    $Configuration=(Resolve-Path -LiteralPath $Configuration).Path
    if($KeepGui -and ($Mode -ne 'Gui' -or $Verify)) { throw 'KeepGui requires a single GUI run.' }
    ...
    Write-CppJson @{baselineRunId=$BaselineRunId;javaHome=$env:JAVA_HOME;configuration=$Configuration;mode=$Mode;keepGui=[bool]$KeepGui;verify=[bool]$Verify} (Join-Path $run 'context.json')
    $arguments=@('-I','-B','-X','utf8',(Join-Path $root 'scripts/g3_integration/runtime.py'),'--root',$root,'--run-id',$RunId,'--mode',$Mode)
    if($Verify) { $arguments+='--verify' }
    if($KeepGui) { $arguments+='--keep-gui' }
    & $PythonExecutable @arguments
    if($LASTEXITCODE -ne 0) { throw "G3 startup failed; see $run/result.json" }
    ...
    Write-Host "G3_T03_RUN_ID=$RunId"
