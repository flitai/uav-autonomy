...
def inspect(root,directory,assignments):
    ...
    for row in assignments:
        ...
        tasks[key]={'definition':task,'kind':kind,'cells':cells,'native':native,'report':node,'seen':set(),
                    'bounds':(min(c['latitude'] for c in cells),max(c['latitude'] for c in cells),
                              min(c['longitude'] for c in cells),max(c['longitude'] for c in cells)),
    ...
    with (directory/'amase/analysis-events.tsv').open(encoding='utf-8') as source:
      for line in source:
        ...
        if node.tag=='AirVehicleConfiguration': configs[node.findtext('ID')]=node
            ...
            for task in tasks.values():
                ...
                for camera in config.findall('PayloadConfigurationList/CameraConfiguration'):
                    ...
                    bounds=task['bounds']
                    # Conservative geographic rejection only: accepted cells
                    # still use the independent polygon and range calculations.
                    if polygon and (max(p[0] for p in polygon)<bounds[0]-1e-12 or
                                    min(p[0] for p in polygon)>bounds[1]+1e-12 or
                                    max(p[1] for p in polygon)<bounds[2]-1e-12 or
                                    min(p[1] for p in polygon)>bounds[3]+1e-12):
                        if task['kind']=='point':task['previous'][sensor]=(milliseconds,False)
                        continue
