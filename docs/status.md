...
**G0、G1、G2 已完成，G2-T01～T07 均已完成；G3-T01～T07 已完成，G3 阶段验收通过；G4 待细化、尚未启动。** 原生 UxAS 干净重建、连续三次启停、中文路径构建／运行、故障矩阵与正式发布通过，见 [T07 报告](g2-uxas-release-validation.md)。
...
| 下一动作 | 用户指定后细化 G4 消息网关；先读 [阶段报告](g3-stage-validation.md) 和 [交接](g3-g4-handoff.md)，重新检查实际资格 |
| 当前边界 | G3 已完成，G4 未启动。无覆盖率门槛、不开展算法／参数寻优；本地观察客户端不是消息网关 |
...
G4 尚未细化或实施，下一项由用户指定。复用前重新核查正式包、同批消息、工具、端口及冻结输入，不将历史身份永久视为有效。GUI 默认 5555／9400／9500，无界面 5556／19400／19500，观察口 9999，两模式顺序运行。默认真实 1 倍、原 785 秒，自动预算 2700 秒，人工等待独立计时。
