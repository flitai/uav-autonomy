...
**G0、G1、G2 已完成，G2-T01～T07 均已完成；G3-T01 输入与验收基线已完成；G3-T02～T04 已完成，G3 尚未完成。** 原生 UxAS 干净重建、连续三次启停、中文路径构建／运行、故障矩阵与正式发布通过，见 [T07 报告](g2-uxas-release-validation.md)。

| 项目 | 当前记录 |
| --- | --- |
| 最近通过的实现任务 | G3-T04：两模式真实规划、分段执行、任务航段推进与正常退出；[报告](g3-execution-validation.md) |
| 源码基线 | T04 起点 `50b282e`，工作区干净；新增独立执行关联／导航探针，T03 保持，正式 AMASE／UxAS、原 XML／模型／生成代码及 43 项冻结输入不变 |
| 已归档历史 | G1-T01～T03 为 `304def9`，G1-T04 为 `f2f73ab`，G1-T05 为 `d77dd78`；T04 `b8ccf72`／`68ed420`、T05 `0fe8553`／交接 `5c2d387` 已推送 origin/main |
| 本轮变更 | [T04 报告](g3-execution-validation.md)：两模式真实任务航段、23 项反例／边界检查通过；最终批次 `g3-t04-test-20260919-121531-644`；工作日志 WL-20260919-006；主提交 `43b4f56` 已普通推送并核对远程一致，随后补齐归档说明 |
...
| 已验证能力 | G0～G2 与 T02 能力保留；T03 暂停启动、连接／真实初始数据屏障、任务初始化后单次请求、非空规划响应、正常关闭、超时与反例；T04 规划／分配／分段命令、内部导航、真实任务航段关联 |
| 尚未验证能力 | 完整任务与覆盖统计正确性、在线自动重连／快照、重置分段、Cesium；Python 其他项目依赖 |
| 下一动作 | [G3-T05](backlog.md#6-g3-顺序与任务卡)：完整任务、可靠完成及覆盖统计正确性；可执行、尚未启动 |
| 当前边界 | G3-T01～T04 已完成；已证明真实分段命令驱动搜索航段，尚未验证完整搜索／可靠末端完成；覆盖统计正确性仍归 T05，不设最低覆盖率 |
...
T01 另确认：原任务 AllAny 在现有覆盖分析分支中直接返回，归 T05 语义核实、统计正确性对照及必要最小修复，不以调参绕过错误；重复 ViewAngleList／旧相机字段归 T02／T05，CMASI 请求无关联 ID，T04 已通过 Unique 身份、分配及分段／轨迹关联。详见 [八项 G3 风险](g3-input-baseline-validation.md#6-已确认风险与后续责任)，本轮没有覆盖率实测值。
...
下一项为 [G3-T05：任务完成与覆盖统计正确性](backlog.md#6-g3-顺序与任务卡)，可执行、尚未启动。复用 T04 最终批次 `g3-t04-test-20260919-121531-644` 与资格 `g3-t01-check-20260919-121013-211`，见 [执行报告](g3-execution-validation.md)。先复查来源，关联实际末端与可靠 TaskComplete，完成 20 米栅格／AllAny 等统计正确性对照和两模式导出；不设最低覆盖率，不以调参绕过统计缺陷。
