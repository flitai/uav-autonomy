...
**G0～G4 已完成；G4-T01～T09 验收通过，正式网关及日常入口已发布。G5-T01～T06 已完成；地图、真实地形后端、状态接入及实体／姿态／时间显示通过，T07 可执行，航线／任务／区域图层待后续。** G5 见 [实施方案](g5-cesium-display-plan.md) 和 [十一张任务卡](backlog.md#9-g5-顺序与任务卡)。G4 方案见 [实施方案](g4-message-gateway-plan.md)；规划提交 `908fdfd` 已推送并核对远程。原生 UxAS 历史正式发布见 [T07 报告](g2-uxas-release-validation.md)。

| 项目 | 当前记录 |
| --- | --- |
| 最近完成的规划 | [G5 方案](g5-cesium-display-plan.md) 已同步 T06 模型尺寸决定、实体／姿态／时间资格、刷新清理及 T07 交接；USGS／Copernicus 及同源高度链保持 |
| 最近通过的实现任务 | G5-T06：AFSIM UCAV 转换、实体／姿态／时间、列表／轨迹／视角／大小快捷键、独立数值及两模式真实显示通过；[报告](g5-entities-validation.md)；[荧光阵营色／描边补充](g5-affiliation-display-validation.md)；[固定尺寸与立体光照](g5-model-display-validation.md)；[逐帧平滑移动](g5-motion-validation.md)；[金属材质预览](g5-metal-display-preview.md)；[近景跟随修正](g5-close-motion-validation.md) |
| 三维模型输入 | 用户提供 16 个 AFSIM OSGB；本轮 UCAV 已转换及校准，1 单位＝1 显示米，可快捷键缩放；其余保持 [输入登记](g5-model-inputs.md) |
| 当前执行结果 | 近景跟随已修正：中文候选 g5-t06-build-20260921-144802-049、普通候选 g5-t06-build-20260921-144826-020；两模式／地图验收 g5-t06-test-20260921-144815-830 passed，见 [屏幕同步证据](g5-close-motion-validation.md)；原 T06 批次保留历史，正式指针不变 |
...
| 下一动作 | 执行 G5-T07：完整航线／命令／执行证据、任务和区域图层；T08～T11 待前置 |
...
先读 [G5 实施方案](g5-cesium-display-plan.md)、[十一张任务卡](backlog.md#9-g5-顺序与任务卡)、[T06 实体验收](g5-entities-validation.md)、[地形后端报告](g5-backend-terrain-validation.md) 和工作日志 WL-20260921-002。T01～T06 已完成，T07 可执行。当前合格三实体地形场景与正式 G4 驱动模型／姿态／时间显示，快捷键尺寸、轨迹／视角及两模式真实浏览器通过；完整航线／任务／区域图层归 T07。日常使用 start-g5-entities-session.ps1，页面关闭不结束后端；观察恢复全矩阵和阶段发布仍归 T08／T11。
