...
**G0～G3 已完成；G4 已细化，G4-T01～T05 已完成，T06 可执行、T07～T09 待前置。** G4 当前方案见 [实施方案](g4-message-gateway-plan.md)；规划提交 `908fdfd` 已推送并核对远程。原生 UxAS 历史正式发布见 [T07 报告](g2-uxas-release-validation.md)。

| 项目 | 当前记录 |
| --- | --- |
| 最近通过的实现任务 | G4-T05 观察恢复与故障拒绝；[报告](g4-recovery-validation.md) |
...
| 本轮变更 | G4 方案 `908fdfd`、T01 `ca21032`、T02 `22cacc6` 已归档；T03 `71c5906` 已归档并发布日志修复；T04 `f3b4d08` 已归档；T05 `g4-t05-test-20260920-004801-605` 六场景及两类反例通过 |
| 当前正式批次 | AMASE `g1-t04-build-20260919-132521-005392`，Finalize `g1-t04-finalize-20260919-154103-934413`；UxAS 构建／验收 `g2-t05-build-20260919-232557-986`／`g2-t05-test-20260919-232811-379`，T07 发布 `g2-t07-publish-20260919-233516-550`；同批 LMCP 不变 |
...
| 尚未完成验收 | G4 两模式完整任务与 20 实体稳定性；G5 Cesium；G6 重置分段；真实地形与 G8 部署；Python 其他项目依赖 |
| 下一动作 | 按 [G4 方案](g4-message-gateway-plan.md) 实施 T06：原两实体 GUI／无界面完整任务及网关对照 |
| 当前边界 | G3 已完成，G4-T01～T05 已完成，T06 可执行。无覆盖率门槛、不开展算法／参数寻优；当前网关已提供只读接口，观察恢复已验收，两模式全程仍待 T06 |
...
用户已确认 [G4 实施方案](g4-message-gateway-plan.md) 与九张顺序任务卡，T01～T05 已完成，T06 可执行。先两实体，再 6 点／8 线／6 矩形的 20 实体混合任务；固定一架一任务，观察链路恢复，两模式各 30 分钟及三个客户端。复用前重新核查正式包、同批消息、工具、端口及冻结输入，不将历史身份永久视为有效。GUI 默认 5555／9400／9500，无界面 5556／19400／19500，观察口 9999，两模式顺序运行。默认真实 1 倍、原 785 秒，自动预算 2700 秒，人工等待独立计时。
