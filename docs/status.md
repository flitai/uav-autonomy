...
**G0、G1 已完成，G1-T01～T05 均已完成；G2-T01 已完成，T02 可执行、尚未启动。** 微软目录来源阻塞已解除，原生工具链十组验收通过；VS／Ninja 在普通及中文空格路径完成真实 C/C++ 编译运行。证据见 [T01 记录](g2-cpp-toolchain-validation.md)。

| 项目 | 当前记录 |
| --- | --- |
| 最近通过的任务 | G2-T01：固定来源、原生安装、十组检查、两个生成器及两类路径下的八个 C/C++ 探针通过 |
| 源码基线 | T01 起点 `48c4d6b`，初始工作区干净；上游源码、G1 实现及生成产物保持不变 |
| 已归档历史 | G1-T01～T03 实现为 `304def9`，G1-T04 为 `f2f73ab`，G1-T05 为 `d77dd78`，均已推送到 origin/main；操作收尾记录另随文档提交归档 |
| 本轮变更 | T01 实现、文档及完成后自动归档约定已随 `78f1056` 推送至 origin/main，远程标识核验一致；本条与实际归档结果另随收尾文档提交归档。首次阻塞见 WL-20260918-003，修复及完成见 WL-20260918-004，本次归档见 WL-20260918-005；此前方案及归档收尾已随 `4f0d4eb`／`48c4d6b` 推送 |
| 已确定的 G2 路线 | MSVC v143＋CMake 3.31、Release x64／动态 CRT；vcpkg manifest 与固定 baseline／overlay；Zyre／串口默认关闭，CZMQ 保留。工具链已验证，业务依赖组合待 T02 验证 |
...
| 已验证能力 | C/C++ 最小程序的 Release x64、动态 CRT、整数／线程／Windows API、路径与故障检查；既有 GUI／无界面 TCP 状态、两层校验、分包回放、中文路径及正常退出 |
| 尚未验证能力 | 第三方业务依赖、C++ LMCP／UxAS 构建、HelloWorld、UxAS 双向网络、完整重连、重置分段验收、原系统闭环、Cesium；Python 其他项目依赖 |
| 下一动作 | 按 G2-T02 固定并构建第三方依赖，建立 manifest／baseline／overlay／triplet 及功能探针；本轮尚未启动 |
| 当前边界 | 仅完成 T01，未开始 T02、重新生成消息或运行仿真；G1 验收保持历史记录，G2 阶段未完成 |
...
| C++／Node 与旧依赖补丁（R02／R10） | T01 完整工具链通过；微软原生签名验证及篡改拒绝解除目录来源阻塞，描述哈希差异原因仍未知；依赖未构建，Node 留待 G5 | T02 核对旧版本配方与补丁，构建必需依赖并执行功能探针 | G2／G5 |
...
- 各任务独立验收并追加根级工作日志；G1 已依据 T01～T05 的历史证据登记完成，不据此宣布原系统闭环通过。G2-T01 完整工具链通过不等于业务依赖、UxAS 或 G2 阶段通过。
- G2 按 [阶段方案](g2-windows-uxas-plan.md)和七张任务卡推进。工具准备／环境与独立探针入口已验证；根级 CMake、业务依赖 vcpkg 清单、UxAS 构建／运行入口仍为拟建内容，只有实际创建并验证后才记为标准命令。

## 下一次工作的起点

下一项为 [G2-T02 固定并构建第三方依赖](backlog.md#g2-t02固定并构建第三方依赖)，可执行、尚未启动；先复用 T01 固定工具，核对原 Anod 版本及补丁，再落实依赖清单和功能验收。T03～T07 继续等待前置任务。2026-09-18 已复核 G1 生成批次 `g1-t03-20260917-152214-037296` 的 588 个 C++ 相关文件，哈希全部匹配；这不是 LMCP C++ 编译结果。
