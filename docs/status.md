...
| 最近通过的实现任务 | G5-T06：AFSIM UCAV 转换、实体／姿态／时间、列表／轨迹／视角／大小快捷键、独立数值及两模式真实显示通过；[报告](g5-entities-validation.md)；[荧光阵营色／描边补充](g5-affiliation-display-validation.md) |
| 三维模型输入 | 用户提供 16 个 AFSIM OSGB；本轮 UCAV 已转换及校准，1 单位＝1 显示米，可快捷键缩放；其余保持 [输入登记](g5-model-inputs.md) |
| 当前执行结果 | 当前配色修订：中文候选 g5-t06-build-20260921-111807-770、普通候选 g5-t06-build-20260921-112011-442；两模式／地图验收 g5-t06-test-20260921-111838-512 passed；原 T06 批次保留历史，正式指针不变 |
