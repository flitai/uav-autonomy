...
**G0、G1、G2 已完成，G2-T01～T07 均已完成；G3-T01～T07 已完成，G3 阶段验收通过；G4 已细化，T01 可执行、实现尚未启动。** 原生 UxAS 干净重建、连续三次启停、中文路径构建／运行、故障矩阵与正式发布通过，见 [T07 报告](g2-uxas-release-validation.md)。

| 项目 | 当前记录 |
| --- | --- |
| 最近通过的实现任务 | G3-T07：最终两模式全程、本轮 GUI 确认、正常退出与 G4 交接；[报告](g3-stage-validation.md) |
| 源码基线 | T07 起点 `6c0294e3`，当时工作区干净；新增独立阶段验收，业务源码／XML／生成代码及 T03～T06 实现保持。inputRevision=3、43 项冻结输入复查通过 |
| 已归档历史 | G1-T01～T03 为 `304def9`，G1-T04 为 `f2f73ab`，G1-T05 为 `d77dd78`；T04 `b8ccf72`／`68ed420`、T05 `0fe8553`／交接 `5c2d387` 已推送 origin/main |
| 本轮变更 | [T07 报告](g3-stage-validation.md)：`g3-t07-test-20260919-173116-474`，固定实际 1 倍，两模式完整任务与覆盖统计通过，当前 GUI 确认及正常退出完成；[G4 交接](g3-g4-handoff.md)，工作日志 WL-20260919-010；主提交 `c12590c` 已普通推送并核对远程一致，随后仅补齐归档说明 |
...
| 下一动作 | 按 [G4 方案](g4-message-gateway-plan.md) 执行 T01；先读 [阶段报告](g3-stage-validation.md) 和 [交接](g3-g4-handoff.md)，重新检查实际资格 |
| 当前边界 | G3 已完成，G4 已细化、实现尚未启动。无覆盖率门槛、不开展算法／参数寻优；本地观察客户端不是消息网关 |
...
用户已确认 [G4 实施方案](g4-message-gateway-plan.md) 与九张顺序任务卡，T01 可执行、实现尚未启动。先两实体，再 6 点／8 线／6 矩形的 20 实体混合任务；固定一架一任务，观察链路恢复，两模式各 30 分钟及三个客户端。复用前重新核查正式包、同批消息、工具、端口及冻结输入，不将历史身份永久视为有效。GUI 默认 5555／9400／9500，无界面 5556／19400／19500，观察口 9999，两模式顺序运行。默认真实 1 倍、原 785 秒，自动预算 2700 秒，人工等待独立计时。
