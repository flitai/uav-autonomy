...
**G0、G1、G2 已完成，G2-T01～T07 均已完成；G3-T01～T06 已完成，T07 可执行、尚未启动；G3 尚未完成。** 原生 UxAS 干净重建、连续三次启停、中文路径构建／运行、故障矩阵与正式发布通过，见 [T07 报告](g2-uxas-release-validation.md)。

| 项目 | 当前记录 |
| --- | --- |
| 最近通过的实现任务 | G3-T06：三次短程实际执行、路径／故障、断线清理与整组重启；[报告](g3-stability-validation.md) |
| 源码基线 | T06 起点 `e1c2c83`，当时工作区干净；本轮仅新增独立编排与验收，正式业务源码／XML／生成代码保持。inputRevision=3、43 项冻结输入及 T05 正式产物复查通过 |
| 已归档历史 | G1-T01～T03 为 `304def9`，G1-T04 为 `f2f73ab`，G1-T05 为 `d77dd78`；T04 `b8ccf72`／`68ed420`、T05 `0fe8553`／交接 `5c2d387` 已推送 origin/main |
| 本轮变更 | [T06 报告](g3-stability-validation.md)：矩阵 `g3-t06-test-20260919-164024-586`，10 个独立子运行、6 项隔离来源、8 项重放对照和总预算检查通过；工作日志 WL-20260919-009。主提交 `b1f5e9b` 已普通推送并核对远程一致，随后补齐归档说明 |
...
| 已验证能力 | G0～G2 与 T02～T04 能力保留；T05 完整任务、可靠 TaskComplete、20 米覆盖统计及独立重放；T06 三次短程实际执行、路径／故障拒绝、断线清理和新组实际执行，正常退出 |
| 尚未完成验收 | T07 阶段全程复验／GUI 人工确认；在线自动重连／快照、重置分段、Cesium；Python 其他项目依赖 |
| 下一动作 | [G3-T07](backlog.md#6-g3-顺序与任务卡) 可执行、尚未启动：冻结配置下分别全程执行、核对完成／覆盖、本轮 GUI 人工确认及正常退出，整理 G4 交接 |
| 当前边界 | G3-T01～T06 已完成；T06 短程与整组重启不替代 T07 全程及人工确认。无覆盖率门槛、不开展算法／参数寻优；G3 阶段尚未完成 |
| 本轮实际倍率 | T06 四组正常短程均为真实 1 倍，运行中及收尾双重核对。T05 正式 GUI 的 1→5→10 倍及用户单次接受保留于其历史报告，不扩大为后续默认策略 |
...
G3-T06 已完成；本轮资格 `g3-t01-check-20260919-162134-898`，矩阵 `g3-t06-test-20260919-164024-586` 和最终原始摘要复查通过，正常四组均实际 1 倍并退出 0，隔离来源与全部业务源码／正式包未变。下一次先读 [T06 报告](g3-stability-validation.md)，按 T07 完成最终两模式全程、本轮 GUI 人工确认及 G4 交接，不能复用 T05 的发布确认或倍率例外。
