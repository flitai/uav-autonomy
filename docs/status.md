...
2026-09-22 侦察覆盖补充已完成：两个图层分别开关，默认开启；当前覆盖用原相机足迹贴同源地形，任务累计覆盖与本次 AMASE 原生统计逐格一致。累计在后台按运行保存，关闭显示继续累计、刷新恢复；不改变任务或仿真。原预览 g5-coverage-session-20260922-113600-734 已正常退出；当前预览 g5-coverage-session-20260922-121323-878 于 12:13 重新启动，真实 1 倍时间、三实体位移和累计覆盖运行身份通过，保留供查看；下一卡仍为 T08。
