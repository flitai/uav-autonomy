...
**G0～G3 已完成；G4 已细化，G4-T01～T06 已完成，T07 可执行、T08～T09 待前置。** G4 当前方案见 [实施方案](g4-message-gateway-plan.md)；规划提交 `908fdfd` 已推送并核对远程。原生 UxAS 历史正式发布见 [T07 报告](g2-uxas-release-validation.md)。

| 项目 | 当前记录 |
| --- | --- |
| 最近通过的实现任务 | G4-T06 原两实体两模式完整任务；[报告](g4-completion-validation.md) |
...
| 尚未完成验收 | G4 三类任务资格与 20 实体稳定性；G5 Cesium；G6 重置分段；真实地形与 G8 部署；Python 其他项目依赖 |
| 下一动作 | 按 [G4 方案](g4-message-gateway-plan.md) 实施 T07：点／线／矩形区域独立小场景和三实体混合资格 |
| 当前边界 | G3 已完成，G4-T01～T06 已完成，T07 可执行。无覆盖率门槛、不开展算法／参数寻优；当前网关已提供只读接口，观察恢复和原两模式全程已验收，混合任务仍待 T07 |
...
用户已确认 [G4 实施方案](g4-message-gateway-plan.md) 与九张顺序任务卡，T01～T06 已完成，T07 可执行。先两实体，再 6 点／8 线／6 矩形的 20 实体混合任务；固定一架一任务，观察链路恢复，两模式各 30 分钟及三个客户端。复用前重新核查正式包、同批消息、工具、端口及冻结输入，不将历史身份永久视为有效。GUI 默认 5555／9400／9500，无界面 5556／19400／19500，观察口 9999，两模式顺序运行。默认真实 1 倍、原 785 秒，自动预算 2700 秒，人工等待独立计时。
