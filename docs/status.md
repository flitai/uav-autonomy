...
**G0、G1、G2 已完成，G2-T01～T07 均已完成；G3-T01 输入与验收基线已完成；G3-T02、T03 已完成，G3 尚未完成。** 原生 UxAS 干净重建、连续三次启停、中文路径构建／运行、故障矩阵与正式发布通过，见 [T07 报告](g2-uxas-release-validation.md)。

| 项目 | 当前记录 |
| --- | --- |
| 最近通过的实现任务 | G3-T03：两模式受控初始化、单次任务／请求、规划响应与正常收尾；[报告](g3-startup-validation.md) |
| 源码基线 | T03 起点 `9325f3b`，工作区干净；新增独立编排／插件，正式 AMASE／UxAS、原 XML／模型／生成代码及 43 项冻结输入不变 |
| 已归档历史 | G1-T01～T03 为 `304def9`，G1-T04 为 `f2f73ab`，G1-T05 为 `d77dd78`；T04 `b8ccf72`／`68ed420`、T05 `0fe8553`／交接 `5c2d387` 已推送 origin/main |
| 本轮变更 | [T03 报告](g3-startup-validation.md)：11 组真实运行／故障、10 项屏障反例及新资格下独立 GUI 暂停／收尾均通过；工作日志 WL-20260919-005；主提交 `7fec15d` 已普通推送，远程核对一致，随后补齐收尾文档 |
...
| 已验证能力 | G0～G2 与 T02 能力保留；T03 暂停启动、连接／真实初始数据屏障、任务初始化后单次请求、非空规划响应、正常关闭、超时与反例 |
| 尚未验证能力 | 自主规划实际执行、完整任务与覆盖统计正确性、在线自动重连／快照、重置分段、Cesium；Python 其他项目依赖 |
| 下一动作 | [G3-T04](backlog.md#6-g3-顺序与任务卡)：关联真实规划、分段命令与 AMASE 任务航段执行；可执行、尚未启动 |
| 当前边界 | G3-T01～T03 已完成；T03 已取消 SendMessagesService 并取得真实规划响应，未验证命令实际执行／完整搜索；覆盖统计正确性仍归 T05，不设最低覆盖率 |
...
下一项为 [G3-T04：规划命令实际执行](backlog.md#6-g3-顺序与任务卡)，可执行、尚未启动。复用 T03 合格流程：矩阵 `g3-t03-test-20260919-113944-818`，独立 GUI 正常收尾 `g3-t03-gui-budget-20260919-114418-444`，新资格 `g3-t01-check-20260919-113233-558`；见 [T03 报告](g3-startup-validation.md)。继续工作先复查来源，关联规划响应、分段命令、CurrentCommand／CurrentWaypoint／AssociatedTasks 与实际轨迹，不把已收到规划响应当作实际执行。
