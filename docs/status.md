...
**G0、G1、G2 已完成，G2-T01～T07 均已完成；G3 待细化、尚未启动。** 原生 UxAS 干净重建、连续三次启停、中文路径构建／运行、故障矩阵与正式发布通过，见 [T07 报告](g2-uxas-release-validation.md)。

| 项目 | 当前记录 |
| --- | --- |
| 最近通过的任务 | G2-T07：完整复验、正式发布及 G3 输入交接；[报告](g2-uxas-release-validation.md) |
| 源码基线 | T07 起点 `8f99ba2`，本地／远程一致且工作区干净；新增独立复验／发布／正式运行脚本，构建、模型、生成代码、算法和原 XML 保持 |
| 已归档历史 | G1-T01～T03 为 `304def9`，G1-T04 为 `f2f73ab`，G1-T05 为 `d77dd78`；T04 `b8ccf72`／`68ed420`、T05 `0fe8553`／交接 `5c2d387` 已推送 origin/main |
| 本轮变更 | T07 验收 `g2-t07-test-20260918-203915-916`、发布 `g2-t07-publish-20260918-204152-283`、正式运行 `g2-t07-run-20260918-204325-360` 均通过；实现提交 `6813d0a` 已普通推送，20:47:56 核对远程完整标识一致。归档结果见 WL-20260918-011，本条随收尾文档提交 |
...
| 尚未验证能力 | AMASE↔UxAS 双向网络、完整重连、重置分段验收、原系统闭环、Cesium；Python 其他项目依赖 |
| 下一动作 | G3 原系统闭环：先细化封装／过滤、启动顺序和 WaterwaySearch 实际命令验收；尚未启动 |
| 当前边界 | G2 已完成，正式 UxAS 已发布；本轮未启动 AMASE、WaterwaySearch 或 G3 |
...
下一项为 G3 原系统闭环，待细化、尚未启动；本轮止于 G2。使用 [T07 报告](g2-uxas-release-validation.md)的正式 UxAS、同批 LMCP、既有 AMASE 与 handoff.json；先按任务约定验证封装、来源过滤、启动顺序和真实命令执行。根 CMake 或其他 LMCP 输入变化时仍需重建／复验，不能改写旧清单。
