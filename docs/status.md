...
**G0～G4 已完成；G4-T01～T09 验收通过，正式网关及日常入口已发布。G5-T01、T02 已完成；USGS 区域同源地形资格通过，T03 可执行，真实地形后端任务与业务显示待后续。** G5 见 [实施方案](g5-cesium-display-plan.md) 和 [十一张任务卡](backlog.md#9-g5-顺序与任务卡)。G4 方案见 [实施方案](g4-message-gateway-plan.md)；规划提交 `908fdfd` 已推送并核对远程。原生 UxAS 历史正式发布见 [T07 报告](g2-uxas-release-validation.md)。

| 项目 | 当前记录 |
| --- | --- |
| 最近完成的规划 | G5 同源真实地形与 Cesium 显示方案、十一张顺序任务卡已确认并落盘；初次方案交付为文档，T01 后续实施已通过 |
| 最近通过的实现任务 | G5-T02 USGS 主地形、WGS84 经纬度统一、同源 DTED／Cesium、Copernicus 对照及八组正式验收；[报告](g5-geography-validation.md) |
| 三维模型输入 | 用户提供 16 个 OSGB 原件，约 154.1 MB，文件摘要已登记；转换及显示校准归 G5-T06，见 [输入登记](g5-model-inputs.md) |
| 当前执行结果 | G5-T02 已完成；准备 g5-t02-prepare-20260920-174145-970、验收 g5-t02-test-20260920-174558-688 passed；原始 z10 失败记录保留，新资格只覆盖区域同源地形 |
...
| 下一动作 | G5-T03 地图与本地资源服务；消费 T02 同批候选和验收收据，完成本地矢量、区域地形、影像切换及独立资源服务验收 |
| 当前边界 | G4 已发布；G5 采用现有本地矢量／DEM、在线影像及 Cesium／AMASE 同源地形，全球浏览与验收区域仿真分开；无覆盖率／FPS 硬门槛，不开展算法／参数寻优 |
...
| AMASE 路径与地形（R06／R08） | 不同工作目录、中文空格路径通过；无 DTED 使用现有零高程缺省值 | 用户已提供全球矢量与 z0～10 DEM，T02 核实数据完整性／垂直基准，T04 另验真实地形计算及统计 | G5-T02／T04 |
...
先读已确认的 [G5 实施方案](g5-cesium-display-plan.md)、[十一张任务卡](backlog.md#9-g5-顺序与任务卡)、[地理检查报告](g5-geography-validation.md) 和工作日志 WL-20260920-014。T01、T02 已完成，T03 可执行，T04～T11 待前置。原始 z10 的 −4037 米异常及失败收据保留；按用户决定，以 USGS 主地形和 Copernicus 对照完成 WGS84 经纬度统一、同源正高／椭球高派生及正式独立验收。下一步完成地图与本地资源服务；真实地形飞行、传感器和统计仍由 T04 验证。下列 G3／G4 记录是继承基线，不代表 G5 整阶段已验收。
