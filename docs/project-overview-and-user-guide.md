...
更新日期：2026-09-23。适用对象：希望了解原项目、当前移植成果以及 Cesium 界面含义的使用者。9 月 23 日补充第 2.5 节界面分工及第 4.3～4.7 节规划能力、操作流程与复用清单，其余实现状态沿用 9 月 22 日核对结果。
...
用户目前启动 `start-g5-coverage-session.ps1` 后，规划会由联合入口自动触发。Cesium 中可以看完整规划、当前命令、执行目标和任务状态；**还不能在网页上画新任务、重新分配飞机或手动点击重新规划**。G6 的仿真开始／暂停控制也不自动等于新增任务编辑与重规划功能，这部分需单独明确范围。
