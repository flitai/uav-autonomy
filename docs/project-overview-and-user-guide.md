...
### 2.5 OpenUxAS 自己有人机交互界面吗

**就本仓库的版本而言，OpenUxAS 本身没有自带完整的地图式图形操作界面。** 它主要以后台服务运行，通过启动参数、XML 配置和 LMCP 消息接收输入，输出规划、命令及日志。[主程序](../OpenUxAS/src/cpp/UxAS_Main.cpp) 是命令行入口；用户通常借助其他程序的界面与它交互。

| 界面或工具 | 属于谁 | 可以做什么 |
| --- | --- | --- |
| 原 AMASE 仿真窗口 | OpenAMASE | 查看飞机、航线和任务，操作仿真时间与运行状态 |
| AMASE Setup Tool | OpenAMASE | 图形化设置场景、实体和部分任务／区域，保存场景文件 |
| 命令行、配置文件和消息接口 | OpenUxAS | 配置规划服务，提供任务与飞机数据，发送规划请求 |
| AutomationDiagram 绘图 | OpenUxAS 的分析辅助工具 | 根据规划响应导出任务、区域和航线图，生成 PDF，也可弹出绘图窗口 |
| 当前 Cesium 网页 | 本项目新增 | 三维态势显示、查询、图层和视角操作；任务编辑与手动规划入口尚未交付 |

上游 [AutomationDiagram 示例](../OpenUxAS/examples/06_AutomationDiagram/README.md) 明确建议借助 AMASE Setup Tool 制作场景，再把事件转换为 UxAS 消息。部分扩展任务需要手工补充字段；绘图工具也有任务与航点显示不完整的限制。它用于理解规划结果，不提供完整的交互式任务编辑和指挥功能；本轮没有在 Windows 重新运行验收该辅助工具。

因此，“原项目有人机界面”和“UxAS 内核没有完整图形主界面”可以同时成立：原系统把人机交互放在 AMASE 或外接控制程序，把自主规划放在 UxAS。当前 Cesium 正在承担新的显示职责，后续控制与任务操作需要通过相应后端接口接入。
...
**原 UxAS 已有任务分配与航线规划能力，当前移植保留并调用了这套能力。** 人提供任务目标、可用飞机和约束，UxAS 生成具体执行方案，AMASE 再计算飞机如何实际飞行。

| 能力 | 输入与计算 | 输出／作用 |
| --- | --- | --- |
| 任务执行方案生成 | 点、线、区域等任务几何，观察要求，候选飞机与传感器配置 | 生成该任务可采用的执行选项及代价，例如从水道哪一端进入 |
| 多机任务分配与排序 | 候选飞机、任务、任务选项、转场及任务代价；可用任务关系表达先后约束 | 决定谁执行哪些任务，以及同一架飞机的执行顺序 |
| 转场路线规划 | 起终点、需要约束的航向、飞机速度／转弯能力、已配置运行区域 | 从飞机当前位置到任务入口，或从上一任务到下一任务的路线与航点 |
| 搜索航线构造 | 地面搜索对象与对应任务服务 | 生成可执行搜索航点；线搜索可以让飞机沿水道侧方飞行并观察水道 |
| 完整计划组装 | 已选分配和各任务实现结果 | 组装各实体的完整计划，形成 `AutomationResponse` |
| 执行阶段管理 | 完整计划及飞机持续报告的当前状态 | 分段下发 `MissionCommand`，在任务执行中更新云台／相机指向 |

原 WaterwaySearch 的 [服务配置](../OpenUxAS/examples/02_Example_WaterwaySearch/cfg_WaterwaySearch.xml) 明确启用了任务管理、请求校验、路线规划、代价汇总、分支定界任务分配、计划组装和航点管理服务。这里的“自主”主要来自这些规划与任务服务，不需要先训练神经网络。

该配置采用 `MINMAX` 分配目标。按当前公开代码的代价计算，它试图减小各飞机累计任务／转场时间中的最大值，可以理解为尽量缩短最晚完成者的预计用时；不是直接把最终覆盖率最大化。计划受输入、代价模型、约束和求解配置影响，不能保证任意场景的实飞覆盖最优。

路线服务可以根据已配置的允许区／禁入区构建平面可视图，并结合需要约束的航向和转弯半径生成航点。这不等于已经具备本轮合格的三维地形避障、动态多机防碰撞或全部区域约束验收；当前 G5 高度适配与这些能力应分开理解。

### 4.4 用原水道示例理解一次规划

原例的 [规划请求](../OpenUxAS/examples/02_Example_WaterwaySearch/MessagesToSend/tasks/1001_AutomationRequest_LINE_Waterway_Deschutes.xml) 指定候选实体 `400、500`，待规划任务 `1000`。这是“两架候选飞机竞争一个任务”，与当前三实体演示的一机一任务不同。

按原 [消息流程说明](../OpenUxAS/doc/reference/UserManual/Examples/doc_WaterwaySearch/WaterwayExample_MessageFLow.tex) 的两端进入示例，UxAS 可以比较“400 从起端进入”“400 从末端进入”“500 从起端进入”“500 从末端进入”等选项，计算转场和执行代价后，选出分配与进入方式，再构造具体航点。另一架没有被分配该任务的飞机继续其原有行为。

```mermaid
flowchart LR
    I["飞机状态、性能、任务与区域约束"] --> Q["AutomationRequest：发起规划"]
    Q --> O["任务服务生成候选方案"]
    O --> C["计算路线与分配代价"]
    C --> A["选择飞机、任务顺序与执行选项"]
    A --> P["生成完整航点计划"]
...
因此，水道折线是人事先给出的任务目标，飞机沿哪条航线执行由 UxAS 计算。场景里也有初始巡航命令；应通过后续规划响应、命令编号和任务关联确认自主规划执行，不能只凭“飞机在动”判断规划成功。

### 4.5 原项目怎样操作，当前移植版怎样操作

原 WaterwaySearch 的操作方式由其 README 和配置共同说明：

1. 准备场景中的飞机初始位置与配置，准备水道任务 XML 和规划请求 XML。
2. 启动 AMASE，打开对应场景，在原 GUI 中按播放按钮，让仿真开始。
3. 启动加载 `cfg_WaterwaySearch.xml` 的 UxAS。
4. 示例中的 `SendMessagesService` 按配置发送预置消息：约启动后 0.3 秒发送线任务、5 秒发送 `AutomationRequest`，触发规划。
5. 等待分配与航点出现；被选中的飞机按命令飞行，到任务航段后相机沿水道观察。

原示例中的规划触发由消息配置完成，README 没有要求在地图上再按“一键规划”。原有 Linux Shell 启动步骤用来理解上游流程；当前 Windows 运行应使用本文第 11 节的合格联合入口。
...
从系统能力看，规划所需输入可以分为四组：飞机当前状态与性能、任务定义、候选实体／任务清单，以及区域与任务关系约束。`AutomationRequest` 是发起计算的消息，主要字段为 `EntityList`、`TaskList`、`TaskRelationships`、`OperatingRegion` 和 `RedoAllTasks`；字段定义见 [CMASI](../OpenUxAS/mdms/CMASI.xml)。任务消息先定义“做什么”，规划请求再指定“本次让哪些飞机考虑哪些任务”。

在当前工程中，更换演示任务应建立独立场景副本，生成对应任务和请求，核查初始化、规划、执行与统计，保留原合格基线。若希望 UxAS 自行选择执行实体，需要让任务的 `EligibleEntities` 与请求的 `EntityList` 允许相应候选飞机；当前三实体场景把这两处都固定为指定单架飞机。

消息协议具备再次请求规划及重做任务的表达，但当前已经验收的启动流程是受控的一次性请求。在线编辑任务后反复规划、旧命令如何替换以及执行中切换的完整流程尚未在当前页面交付；不能把刷新浏览器当作重新规划。

### 4.7 哪些能力可以继续复用

用户已确定项目方向为**具有侦察监视任务库的通用自主任务规划框架**。任务操作界面和后端接口由本项目补齐，计划流程为“任务草稿 → 校验并请求规划 → 方案审查 → 明确确认下发 → 执行跟踪 → 按需受控重规划”。见 [G6 后续计划](g6-task-planning-interaction-plan.md)；这是未来功能安排，当前页面仍未提供这些操作。

当前点／线／矩形搜索，以及请求校验、任务选项、路线代价、任务分配、完整计划组装和分段命令链已经在实际运行中使用。上游还具备指定方向区域搜索、螺旋／扇区／扫掠模式搜索、持续监视、多机监视、伴随侦察、必经点、盘旋、集结同步和通信中继等任务实现，需要逐项接入并验证。

完整的“能力—实现—已验范围—接入工作”对照见 [UxAS 规划能力复用清单](uxas-planning-reuse.md)。复用的重点是继续通过消息使用已有后端，同时补充 Cesium 任务输入与操作流程。若增加“规划预览后确认执行”，还须控制当前会自动下发命令的执行链；不能仅增加一个预览按钮。
...
| 拖动时间查看历史 | G7 待实施 |
| 任务编辑、规划预览／确认下发、自由分配与受控重规划 | 已列入 G6-B 八张后续任务卡，尚未实现；首批点／线／矩形搜索 |
| 更多侦察监视任务 | G6-B 后按任务类型逐项接入和验证；参见后续计划的任务库扩展 |
| 量测、新避障算法、覆盖效果寻优 | 当前未交付，按后续实际需求实施 |
