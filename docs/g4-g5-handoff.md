...
G5 已按用户确认的 [实施方案](g5-cesium-display-plan.md) 细化为 [十一张任务卡](backlog.md#9-g5-顺序与任务卡)，T01 环境及最小工程已完成，见 [环境报告](g5-environment-validation.md)；T02 已完成 USGS 区域同源地形资格，见 [地理检查报告](g5-geography-validation.md)；T03 地图与本地资源服务、两类路径及生产／开发、断网／在线矩阵通过，见 [地图报告](g5-map-validation.md)；T04 两模式三实体真实地形任务及独立统计已通过，见 [后端地形报告](g5-backend-terrain-validation.md)。T05 可执行，真实业务接入尚待实施。G5 新增本地矢量／DEM、在线卫星影像及 Cesium／AMASE 同源真实地形；后端零高程基线保持历史资格。T04 原平面几何和任务身份保留，共同地形赋高、统一 +390 米飞行高度适配已冻结，不属于覆盖效果寻优；当前非负区域的实际路径避开旧 DTED 间距字段缺陷，负值继续拒绝，相机四角仍为局部平面近似。后续真实链路须绑定本卡场景／地形和正式后端，原两实体／20 实体完整显示联调归 T09／T10。交互控制和重置归 G6，正式历史回放归 G7，第二机器与离线影像包归 G8；算法／覆盖效果优化以及任意 Unicode 业务字符串兼容不在 G5 范围。
