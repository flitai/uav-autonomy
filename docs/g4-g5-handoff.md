...
G5 已按用户确认的 [实施方案](g5-cesium-display-plan.md) 细化为 [十一张任务卡](backlog.md#9-g5-顺序与任务卡)，T01 环境及最小工程现已完成，见 [环境报告](g5-environment-validation.md)；T02 已按后续决定完成 USGS 区域同源地形资格，见 [地理检查报告](g5-geography-validation.md)，T03 地图与本地资源服务后续已完成，两类路径及生产／开发、断网／在线矩阵通过，见 [地图报告](g5-map-validation.md)；T04 可执行，真实业务接入尚待后续。G5 在只读真实显示之外新增本地矢量／DEM、在线卫星影像及 Cesium／AMASE 同源真实地形；后端零高程基线保持历史资格，真实地形须独立验证飞行、传感器与统计。固定平面网格和任务身份保留，地面赋高及必要飞行高度适配按 G5 方案冻结；这不属于覆盖效果寻优。交互控制和重置归 G6，正式历史回放归 G7，第二机器与离线影像包归 G8；算法／覆盖效果优化以及任意 Unicode 业务字符串兼容不在 G5 范围。
