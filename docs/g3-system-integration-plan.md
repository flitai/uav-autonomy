...
G3 的通过条件提升为：**Windows 原生 GUI、无界面均完成 WaterwaySearch 搜索任务，实际执行链完整，AMASE 搜索覆盖率 ≥95%，进程正常退出。** 此标准在本轮由用户确认，替代总体计划此前“规划命令实际影响运行”的最低门槛。
...
UxAS TCP 观察口用于读取总线消息、注入验收任务／请求以及正常停止请求。观察客户端只发送明确的编排消息，不转发所收业务消息；任务和规划请求各有单次触发记录，实际 MissionCommand 由 UxAS 生成。观察记录与 AMASE 内部事件、真实收发样本交叉核对。
...
默认 GUI／无界面均使用 1 倍速。原始场景先按 785 仿真秒运行；受控调优副本最多 1800 仿真秒。初始化阶段、任务初始化／规划等待及正常关闭各设 30 秒超时，整次自动运行墙钟上限 2700 秒。GUI 自动运行到验收观察点后可进入独立的人工确认等待状态，等待人工不算仿真运行或自动成功；确认后仍须验证正常退出。任何超时、强制终止或未完成条件都保留失败／待确认状态。
...
覆盖率复用 AMASE 现有计算语义。无界面导出隔离进度窗口，补充原始计数和漏覆盖栅格位置，保留分析 XML；使用 `total > 0` 且 `100 × seen ≥ 95 × total` 判定，不用格式化百分比跨过门槛。每次运行独立统计，两模式分别达标，不合并不同候选、模式或运行的覆盖结果。

先记录原始参数结果；未达标时最多比较三组受控候选。每组保存完整配置、相对原示例的参数差异、调整依据、任务执行与覆盖变化。可调范围为飞行、传感器、规划参数及场景时长；完整水道、实体身份、20 米栅格、覆盖计算语义和 95% 门槛固定。冻结一组同时适用于 GUI／无界面的合格业务参数，模式差异仅限显示、端口和编排设置。

不得修改评分算法、删除未覆盖水道段、伪造完成事件或把不同运行覆盖率拼接为达标。三组候选仍未达标、需要超出 1800 秒或改变任务范围时，保留结果并登记阻塞，明确原因、恢复条件和下一次最小实验。
...
| G3-T05 任务完成与覆盖率达标 | T04 | 完成判定、分析导出、原参数与受控调优报告 | GUI／无界面各自完成任务且覆盖率 ≥95%，冻结合格配置 |
| G3-T06 稳定性与故障复验 | T05 | 重复／路径／故障矩阵及整组重启证据 | 三次短程闭环正常启停，故障明确失败，重启不混入旧运行 |
| G3-T07 阶段验收与 G4 交接 | T06 | 最终两模式全程复验、人工确认、唯一配置及交接清单 | 全部完成条件、覆盖率及正常退出通过，才登记 G3 完成 |
...
- 新增独立 G3 编排、验证插件、测试和配置目录；具体可复制命令在对应任务真正实现并验证后才写入报告。现有 G1／G2 接口及原示例保留，MDM／生成代码不因编排而改变。
...
| 完成与覆盖 | 提前 TaskComplete、错误实体／任务、缺少末端、空栅格、阈值下方与四舍五入边界、两模式全程 | 每次运行独立满足完成链和 ≥95%；保留失败基线和候选 |
