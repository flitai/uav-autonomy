...
实施补充（2026-09-18）：以上为编制时的分析基线；G0、G1 已完成，G1-T01～T05 均已完成，G2-T01 已完成，MSVC／SDK 及便携工具十组验收、两个生成器和两类路径下的 C/C++ 探针通过，T02 已完成 89 个 ports 的源码重建、功能探针与迁移发布，T03 可执行、尚未启动，详见 [T01 记录](docs/g2-cpp-toolchain-validation.md)与 [T02 记录](docs/g2-dependencies-validation.md)；阶段路线见 [G2 阶段方案](docs/g2-windows-uxas-plan.md)和 [七张任务卡](docs/backlog.md#4-g2-顺序与任务卡)。正式 AMASE、GUI／无界面真实 TCP 接收、十四组自动验收、人工确认、同版本受控复验及正常退出均通过，见 [T04 报告](docs/g1-amase-validation.md)、[T05 报告](docs/g1-tcp-validation.md)及 [当前状态](docs/status.md)。无界面闪屏／错误提示作了最小适配，实体 TCP 端口必须随实例隔离；已验证 AMASE→Python 接收，不据此宣布 UxAS／Cesium 或双向网络闭环完成。
