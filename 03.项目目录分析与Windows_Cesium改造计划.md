...
编制日期：2026-09-16  
分析对象：`E:\uav-autonomy` 当前工作区，根仓库提交 `061685f`（`init`）。  
...
| P0 | 当前 UxAS TCP 桥使用 attributed message 与 Sentinel 封装；AMASE TCP 服务读取原始 LMCP 包 | 存在协议兼容风险，必须先做双向字节级验证，详见第 5 节 |
...
| 端点 | 当前配置角色 | 协议与用途 |
| --- | --- | --- |
| AMASE TCP `5555` | AMASE 服务端，UxAS 配置为客户端 | AMASE `TcpServer` 使用 `LMCPFactory` 读写原始 LMCP 二进制包 |
...
因此，直接照旧配置将两端连在 `5555` 上，存在明显的封装不匹配风险。应以两端抓取的真实字节和往返测试确定表现，不能仅以“端口连通”判定成功。源码依据：[TCP 桥](OpenUxAS/src/cpp/Communications/LmcpObjectNetworkTcpBridge.cpp)、[属性消息收发](OpenUxAS/src/cpp/Communications/ZeroMq/ZmqAttributedMsgSenderReceiver.cpp)、[Sentinel 发送器](OpenUxAS/src/cpp/Communications/ZeroMq/Senders/MsgSenderSentinel.cpp)。

优先方案是在 UxAS 中增加或恢复明确的 **AMASE 原始 LMCP TCP 模式／专用桥**，保留现有 attributed TCP 用途。若首次移植需要暂时绕开该路径，可用独立适配器连接 AMASE `5555` 与 UxAS `5560/5561`，但必须：
...
| 能力 | 实施要求 |
| --- | --- |
| 解码 | TCP 按 LMCP 头部长度增量分包，处理半包／粘包、校验和、未知类型与断线残包；ZeroMQ 按实际外层封装解包 |
