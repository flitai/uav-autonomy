"""Indexed independent G5 terrain statistics replay for the frozen mixed20 scene.

Ground height comes from independently decoded DTED at the aircraft position,
matching the formal SearchTaskAnalysis AGL contract. Footprint polygon geometry
is native local-plane output, not four independent terrain intercepts. The
0.001-degree buckets only discard cells outside a camera footprint bounding
box; every candidate still receives the original independent geometry test.
"""
import base64
...
def inspect(root,directory,assignments,terrain):
    ...
    for row in assignments:
        ...
        for index,cell in enumerate(cells):
            bucket=(math.floor(cell['latitude']*1000),math.floor(cell['longitude']*1000))
        ...
        tasks[key]={'definition':task,'kind':kind,'cells':cells,'native':native,'report':node,'seen':set(),
                    ...
                    'seenByEntity':{},'intervalsByEntity':{},'buckets':buckets}
    ...
    for line in (directory/'amase/analysis-events.tsv').read_text().splitlines():
        ...
        if node.tag=='AirVehicleConfiguration': configs[node.findtext('ID')]=node
            ...
            ground=terrain.nearest(*position)
            need(ground>0 and altitude-ground>=100-0.001,'invalid-real-flight-terrain-clearance')
            altitude-=ground
            for task in tasks.values():
                ...
                for camera in config.findall('PayloadConfigurationList/CameraConfiguration'):
                    ...
                    # CameraControl intentionally emits zero for these planar corner heights.
                    # Verify that field contract without interpreting it as terrain elevation.
                    need(all(float(p.findtext('Altitude'))==0 for p in footprint),'native-footprint-height-contract-changed')
                    ...
                    qualified=False
                    candidates=[]
                    if band and len(polygon)>=3:
                        south,north=min(p[0] for p in polygon),max(p[0] for p in polygon)
                        west,east=min(p[1] for p in polygon),max(p[1] for p in polygon)
                        for (latitude,longitude),indices in task['buckets'].items():
                            if (math.floor(south*1000)<=latitude<=math.floor(north*1000) and
                                    math.floor(west*1000)<=longitude<=math.floor(east*1000)):
                                candidates.extend(indices)
                    for index in candidates:
                        if task['kind']!='point' and index in seen_by_entity: continue
                        cell=task['cells'][index];lat,lon=cell['latitude'],cell['longitude']
                        visible=geometry.inside(lat,lon,polygon)
    ...
    return {'status':'passed','tasks':results,'minimumCoveragePercent':None,'minimumObservationSeconds':None,'terrain':'qualified-USGS-DTED-EGM96; nearest at aircraft position'}
