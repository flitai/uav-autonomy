...
async def watch(url, output, run_id):
    ...
    async def client(index):
        ...
        with (output / ('client-' + str(index) + '.jsonl')).open('w', encoding='utf-8') as log, \
                (output / ('latency-' + str(index) + '.jsonl')).open('w', encoding='utf-8') as latency:
                        ...
                        while not (output / 'request-stop').exists():
                            ...
                            if state is None:
                                ...
                                for change in packet['changes']:
                                    if change['collection']=='entities' and change['op']=='upsert' and 'state' in change['value']:
                                        value=change['value']['state']
                                        latency.write(json.dumps({'entity_id':change['id'],'source_time_ms':value['Time'],
                                            'received_monotonic':time.monotonic(),'stream_id':stream_id})+'\n')
        ...
        # A slow reader may skip an entire intermediate process incarnation.
        # Reconnection promises a current snapshot, not every historical stream.
        minimum_streams=2 if index==2 else 3
        assert result['snapshots'] >= minimum_streams and len(result['streams']) >= minimum_streams and result['deltas'] > 0, result
