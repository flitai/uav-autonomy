...
$runId='g5-t10-test-'+(Get-Date -Format 'yyyyMMdd-HHmmss-fff')
...
$record=[ordered]@{task='G5-T10';runId=$runId;status='running';buildRunId=$BuildRunId;fullRunId=$FullRunId}
try {
    ...
    & $PythonExecutable -I -B -X utf8 (Join-Path $root 'scripts/g5_scale/manage.py') --run-id $runId --baseline-run-id $BaselineRunId --build-run-id $BuildRunId --full-run-id $FullRunId
    if ($LASTEXITCODE -ne 0) {throw 'G5-T10 failed; inspect result.json'}
    ...
    Write-Host ('G5_T10_RUN_ID='+$runId)
}
