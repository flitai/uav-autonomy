...
$runId='g5-t08-test-'+(Get-Date -Format 'yyyyMMdd-HHmmss-fff')
...
$record=[ordered]@{task='G5-T08';runId=$runId;status='running';buildRunId=$BuildRunId}
try {
    ...
    & $PythonExecutable -I -B -X utf8 (Join-Path $root 'scripts/g5_lifecycle/manage.py') --run-id $runId --baseline-run-id $BaselineRunId --build-run-id $BuildRunId
    if ($LASTEXITCODE -ne 0) {throw 'G5-T08 failed; inspect result.json'}
    ...
    Write-Host ('G5_T08_RUN_ID='+$runId)
}
