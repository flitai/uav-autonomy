...
string(REPLACE ".known-versions = 14.2" ".known-versions = 14.3 14.2" _contents "${_contents}")
file(WRITE "${CURRENT_PACKAGES_DIR}/tools/boost-build/src/tools/msvc.jam" "${_contents}")

# This Windows/MSVC-only overlay emits one BOM per response file, after Jam
# list expansion (putting it in the action duplicates it for every object).
file(READ "${CURRENT_PACKAGES_DIR}/tools/boost-build/src/engine/function.cpp" _contents)
string(REPLACE "string_free( out_name );" [=[
if ( out_name->size >= 4 && strcmp( out_name->value + out_name->size - 4, ".rsp" ) == 0 )
                    fputs( "\xEF\xBB\xBF", out_file );
                string_free( out_name );]=] _contents "${_contents}")
file(WRITE "${CURRENT_PACKAGES_DIR}/tools/boost-build/src/engine/function.cpp" "${_contents}")
...
if(CMAKE_HOST_WIN32)
    ...
    # CMake/Jam files are UTF-8. Make the old engine's narrow Windows APIs
    # use the same encoding, without changing the machine locale.
    find_program(UXAS_MT mt.exe REQUIRED)
    vcpkg_execute_required_process(
        COMMAND "${UXAS_MT}" -nologo -manifest "${CMAKE_CURRENT_LIST_DIR}/utf8.manifest"
            "-outputresource:${CURRENT_PACKAGES_DIR}/tools/boost-build/b2.exe;#1"
        WORKING_DIRECTORY "${CURRENT_PACKAGES_DIR}/tools/boost-build"
        LOGNAME utf8-manifest-${TARGET_TRIPLET}
    )
else()
    vcpkg_execute_required_process(
