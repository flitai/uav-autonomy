...
G3 终点为 GUI／无界面分别完成 WaterwaySearch 任务执行，实际执行链可靠、AMASE 20 米栅格覆盖计算与报告正确、本轮 GUI 人工确认及正常退出。用户已明确当前重点是调通程序和系统，不设最低覆盖率，不开展为达指标的算法／参数寻优；覆盖效果优化归后续。完整水道保留，允许必要的功能兼容和统计正确性修复。基础断线清理及整组重启归 G3，自动重连／快照补齐归 G4，重置分段归 G6。T01 独立输入资格入口已验证，T02 独立协议编排／探针及通信修复已完成正式自动验收、本轮 GUI 确认和复验发布，见 [T02 报告](docs/g3-protocol-validation.md)；T03 已通过两模式真实初始化、单次任务请求、非空规划响应和正常收尾，见 [启动报告](docs/g3-startup-validation.md)；T04 已通过两模式分段命令、内部导航与任务航段关联及正常退出，见 [执行报告](docs/g3-execution-validation.md)；T05 已完成正式两模式全程、可靠 TaskComplete、统计独立复算和正常退出，见 [T05 报告](docs/g3-completion-validation.md)；T06 已完成，见 [稳定性报告](docs/g3-stability-validation.md)；T07 已完成，见 [阶段报告](docs/g3-stage-validation.md)；G4-T01～T09 已完成，G4 已验收并发布；G5-T01～T07 已完成；地图、真实地形后端、状态接入、实体／姿态／时间及航线／任务／区域显示通过，T08 可执行，完整恢复矩阵和阶段发布待后续。后续先读 [阶段报告](docs/g3-stage-validation.md)和 [G4 交接](docs/g3-g4-handoff.md)；[T01 输入基线](docs/g3-input-baseline-validation.md)中的风险为历史发现，处理结果以当前状态和各卡证据为准。
...
依赖包只验收 Release x64／动态 CRT。CZMQ 旧 `snprintf` 宏和 CMake 3.31／Ninja 的中文响应文件处理见 T02 报告，固定依赖已通过 T05 的 UxAS 完整编译与候选验收。G2、G3 均已完成，见 [G3 阶段报告](docs/g3-stage-validation.md)；G4-T01～T09 已完成，G4 已验收并发布；G5-T01～T07 已完成；地图、真实地形后端、状态接入、实体／姿态／时间及航线／任务／区域显示通过，T08 可执行，完整恢复矩阵和阶段发布待后续。完整任务与稳定性证据另见 [完成报告](docs/g3-completion-validation.md)和 [稳定性报告](docs/g3-stability-validation.md)。
...
默认 config/g3-execution.json，独立 g3-t04- 运行编号；接受 Configuration 和可选 BaselineRunId。验证两真实执行分段、四个任务目标及至少两段实际推进后正常关闭，不等待完整任务。原示例实际 TurnShort、四点重叠均保留；不能把网络状态跳号直接当成遗漏执行，须核对内部导航。完整完成及覆盖统计已由 T05 另行验收，不设覆盖率门槛；T07 已另行取得最终全程 GUI 人工确认及正常退出。
...
本轮四项 AMASE 源码修复已重建，通过 11 组自动复验、用户真实 GUI 确认、正常退出和发布；未变更 UxAS 二进制按既有 T07 流程复验发布以更新 AMASE handoff。inputRevision=3 保留旧修订和摘要，当前资格与正式两模式收据见 T05 报告。T06 已完成，见 [稳定性报告](docs/g3-stability-validation.md)；T07 已完成，见 [阶段报告](docs/g3-stage-validation.md)；G4-T01～T09 已完成，G4 已验收并发布；G5-T01～T07 已完成；地图、真实地形后端、状态接入、实体／姿态／时间及航线／任务／区域显示通过，T08 可执行，完整恢复矩阵和阶段发布待后续；G3 阶段全程 GUI 人工确认及正常退出已完成。
...
T06 已通过三次连续短程实际执行（含 GUI、中文空格路径和其他工作目录）、六项隔离来源拒绝、端口冲突、两种提前退出、规划／关闭超时、真实半帧断线清理及新编号整组重启。底层故障收据保持 failed，上层单独判断预期拒绝；正常四组均为真实 1 倍、退出 0、无强制终止且端口释放。详见 [T06 报告](docs/g3-stability-validation.md)。
...
配置为 config/g3-stability.json；单次使用 run-g3-stability.ps1，可指定 Mode／ChinesePath，故障诊断另有 TestFault。CompletionRunId 必须对应当前正式产物和冻结配置的合格 T05；省略 BaselineRunId 时重新做输入资格检查，不能永久复用历史身份。当前未修改业务源码、正式包或 T03～T05 入口。短程不能代替全任务，T07 已另行完成最终两模式全程和本轮 GUI 人工确认／正常退出；在线重连／快照归 G4。

### Windows：G3 阶段验收与 G4 交接

T07 已通过最终两模式完整任务、可靠 TaskComplete、20 米逐格复算及同记录重放；实际 SessionStatus 全程 1 倍，本轮 GUI 确认后正常关闭，所有本次进程退出 0、端口释放。G3 已完成；标准入口复查当前资格、T01～T06 收据和冻结配置，详见 [阶段报告](docs/g3-stage-validation.md)与 [G4 交接](docs/g3-g4-handoff.md)。
...
# 本次 GUI 完成并取得真实用户确认后，绑定该次运行收尾：
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\windows\finish-g3-acceptance.ps1 -PythonExecutable $pythonExe -RunId '<G3_T07_RUN_ID>' -ConfirmationText '<本次用户回复>'
```
...
没有新增业务源码／生成输入，正式包和历史收据保持；不设最低覆盖率、不开展寻优。G4 已按 [实施方案](docs/g4-message-gateway-plan.md) 完成九张任务卡并发布；G3 观察工具与 G4 独立网关分开，G4-T05 已验证观察重连和语义补齐；重置分段归 G6，零地形、Unicode 字段及第二机器部署限制保持。

### Windows：G4 网关环境与契约

G4-T01～T09 已完成；[环境报告](docs/g4-environment-validation.md) 和 [v1 契约](docs/g4-browser-contract.md)。`scripts/windows/setup-g4.ps1 -PythonExecutable <已核查解释器>` 重新检查正式来源、创建并验收独立 Web 环境；`-VerifyOnly` 只复查已有环境。`tests/windows/g4-environment.tests.ps1` 接受 PythonExecutable 和本次 BaselineRunId，验证中文工作目录、重复编号保护和缺解释器拒绝。环境指针在 `.tools/g4/current.json`，完整版本／wheel 哈希见 config/g4-python-lock.json；调用前核查来源和安装文件。该环境冒烟本身未启动仿真，真实网关与在线恢复另由后续任务证据证明。

### Windows：G4 严格协议与真实接入

G4-T02 已完成；[报告](docs/g4-protocol-validation.md)。`tests/windows/g4-protocol.tests.ps1 -PythonExecutable <已核查解释器>` 顺序验证两模式，单模式入口为 run-g4-protocol.ps1，接受 Mode／BaselineRunId。src/sim_bridge 已有独立严格协议核心和有界只读接收器；七 MDM 字节预检、两层校验、来源、实际动态状态和真实规划消息与 G3 独立观察对照通过。该短程不验证完整任务、浏览器或在线恢复；满队列／半帧保留失败记录，网关不发送业务帧。T03 状态模型与持久重建已通过，见 [T03 报告](docs/g4-state-validation.md)。`tests/windows/g4-state.tests.ps1 -PythonExecutable <已核查解释器>` 复验两模式在线日志读取和重建；省略 BaselineRunId 时取得新资格。

G4-T03 修复只读查询引发 UxAS 日志丢失的问题：正式日志写端启用 WAL、有限等待及正确失败返回，网关仍只读。新正式发布 g2-t07-publish-20260919-233516-550、新资格 g3-t01-check-20260919-233710-372，T03 两模式 g4-t03-test-20260919-233834-574 通过；当前版本以 config/g4-baseline.json 的独立修订及正式 current.json 为准，不覆盖 G3 handoff。当前状态来自有序持久日志，原始 TCP 捕获另存；T04 已通过独立 HTTP／WebSocket、两模式三客户端及本机 Edge 页面渲染／刷新／正常退出；见 [T04 报告](docs/g4-web-validation.md)。`tests/windows/g4-web.tests.ps1 -PythonExecutable <已核查解释器>` 为短程验收入口；默认本机 8000，只读，浏览器无后端控制路径。自动观察恢复仍归 T05。
...
### Windows：G4 观察恢复与故障拒绝

G4-T05 已完成；[恢复报告](docs/g4-recovery-validation.md)。`tests/windows/g4-recovery.tests.ps1 -PythonExecutable <已核查解释器>` 验证原例无界面真实完成跨缺口补齐、GUI 观察恢复、三持续客户端、主链路／后端退出拒绝及新整组运行。单模式诊断入口为 run-g4-recovery.ps1，接受 Mode／BaselineRunId；GUI 单模式只做恢复短程，不能当作完整任务通过。
...
G4-T06 已完成；[完成报告](docs/g4-completion-validation.md)。`tests/windows/g4-completion.tests.ps1 -PythonExecutable <已核查解释器>` 顺序完成原例 Headless／Gui 全程，单模式入口 run-g4-completion.ps1；每模式均保留 400／500 和已完成任务 1000，实际 1 倍、完整水道、20 米栅格及无覆盖门槛保持。独立 `scripts/g4_completion/audit.py --root <仓库根目录> --run-id <本次双模式编号>` 核查原始 Running 倍速、来源／证据摘要及零原始完成帧对应唯一持久完成事件，另写新审计收据。两份收据齐全才复用本卡结果；不改写父运行。

本卡三客户端、半帧观察断线、网关重启、完成事件补齐、真实 Edge 刷新及所有所属进程正常退出通过；自动页面检查不代替 T09 本轮人工 GUI 确认。
...
G4-T08 已完成，见 [稳定性报告](docs/g4-scale-validation.md)。固定 8 线／6 点／6 矩形、20 架各一任务；取证 g4-t08-capture-20260920-085508-125483 与独立审计 g4-t08-audit-20260920-100537-000770 passed，两模式各 1800 仿真秒、真实 1 倍、三客户端、逐架执行／统计、观察恢复及正常退出通过。T09 当前 GUI 确认、同版本完整受控复验及网关合格发布已完成，见 [阶段报告](docs/g4-stage-validation.md)。
...
- **CesiumJS**：三维显示和用户交互；T03 本地矢量、区域地形与在线影像已通过地图资源验收，T05 真实接入与 T06 实体／姿态／时间已通过。浏览器掉帧、相机操作或底图变化不能改变仿真结果。
...
- models/ 的 16 个 OSGB 已登记原件；T06 已转换 AFSIM UCAV，用户确认 1 源单位＝1 显示米，模型轴／原点及姿态已校准，资产修正与后端姿态分开。显示模型不改变实体仿真能力；未选用资产不增加场景验收范围。
...
G4-T01～T09 已完成：原两实体全程、点／线／区域资格、20 实体两模式各 30 分钟、三客户端、观察连接和网关恢复、当前 GUI 确认及正常退出均有独立证据。正式网关指针为 out/artifacts/gis-gateway/current.json，日常入口资格为同目录 session.json；来源、运行编号和本轮同版本受控收尾见 [阶段报告](docs/g4-stage-validation.md)。G5-T01～T07 已完成，T08 可执行；见 [实施方案](docs/g5-cesium-display-plan.md)和 [十一张任务卡](docs/backlog.md#9-g5-顺序与任务卡)。
...
浏览器只读 http://127.0.0.1:8000；根运行目录 request-stop 正常结束本组。网关自行退出后，编排以同一运行和事件库自动恢复网关，后端持续运行；需要暂留观察离线时先创建根目录 gateway-offline，再在当前 observer 输出目录创建 request-stop，移除 gateway-offline 后自动恢复。后端或主任务链路失效仍失败并整组重启。run-g4-gateway.ps1 可独立启动只读网关，但不能与自动管理同一端口的会话同时使用；run-g4-session.ps1 为冻结的历史取证入口，日常使用 start-g4-session.ps1。
...
G5 用户后续决定采用原全球矢量＋USGS 区域主地形、Copernicus 对照，Esri 按切换加载；T01～T07 已通过，T08 可执行，当前方案及十一张卡已同步实际证据和已知限制。G4 零高程历史结果不替代真实地形任务验收；来源／基准／覆盖缺失不能静默使用零高程。T03 代理采用协议夹具，T05 真实首次／晚加入／刷新已通过，完整故障隔离另由 T08 验证；T06 选用模型已合格，最终人工确认及正式前端发布归 T11。查询／图层／视角操作归 G5，控制与重置归 G6，历史回放归 G7，第二机器和离线卫星影像包归 G8。

## G5-T05 前端状态与真实接入

T05 已完成，见 [状态接入报告](docs/g5-state-validation.md)。apps/cesium_state 为独立 TypeScript 协议／归约器／只读连接模块，与未变的 T03 生产地图资产组合为新候选；不改写 T02～T04 或正式 G4 收据。32 项协议／缓存检查、两模式真实首次／晚加入／刷新、独立收到帧重放、暂停全量快照对照和正常退出通过。地图就绪与后端同步分开；实体模型和业务几何仍待 T06／T07。

构建使用 `scripts/windows/build-g5-state.ps1`（可加 ChinesePath），验收使用 `tests/windows/g5-state.tests.ps1`，联合运行使用 `scripts/windows/start-g5-state-session.ps1`（Mode Gui／Headless）。均需 PythonExecutable，验收／运行需 BuildRunId，可省略 BaselineRunId 重新复查来源。联合入口启动 T04 三实体场景、正式 G4 和 8080 页面；创建输出根目录 request-stop 正常结束本组，浏览器关闭不结束后端。`run-g5-state.ps1` 仅启动页面服务；外部后端的地形身份不能据此自动合格。报告保存实际候选／验收／联合入口编号及当前验证范围。

每次 WS 连接先快照后连续增量，ID／时间／序号保留字符串并用 BigInt 比较，完整 upsert／显式 delete。失配或重连清空业务和轨迹，只显示标为过期的最后时间；不拼接 HTTP snapshot，不外推时钟或发送业务控制。业务 JSON 单包 8 MiB、每集合 4096 对象、总计 32 MiB；轨迹最多 128 实体，每实体 2048 点／10 分钟，固定字段且按后端时间清理，完整规划独立保留。

真实浏览器关闭暴露 T03 资源服务未处理客户端 ECONNRESET；T05 服务包装入口补充每个客户端套接字错误处理与有界计数，六次主动重置和实际断开后正常运行、正常退出通过，旧 T03 源码保持。独立浏览器使用绝对 profile，刷新和截图前激活受检页，地图与业务都就绪后取证，异常路径仍正常 Browser.close。T05 只登记 realStateConnectionQualified，完整显示、T08 恢复矩阵和 T11 人工确认／发布继续待后续。

## G5-T04 真实地形后端资格

T04 已完成，见 [后端地形报告](docs/g5-backend-terrain-validation.md)。正式 AMASE／UxAS 和 G4 来源保持；独立 original／small／mixed20 场景按共同 EGM96 网格赋高，完整水道及平面几何保留，统一抬升 390 米。三实体点／线／区域在 Headless／Gui 真实 1 倍完成，实际净空、内部执行、可靠完成和独立 20 米统计／点观察时间通过，所属进程正常退出。

入口为 `scripts/windows/prepare-g5-backend.ps1`、`scripts/windows/run-g5-backend.ps1`、`tests/windows/g5-backend.tests.ps1`，均需明确 PythonExecutable；run 需 BuildRunId，test 需 BuildRunId／FlightRunId。默认两模式，单模式只作诊断，独立资格要求两模式；完整命令及当前三编号见报告。每次核查上游资格、37 项输入和候选文件，新增实现不改写旧收据。固定端口及预算见 config/g5-backend-terrain.json；本卡不会切换任何正式指针。

实际 DTED 读取来自正式 JAR，运行包装器只委托、取证并拒绝缺图／越界／负值；旧负数解码和十倍间距元数据缺陷未修复，当前非负区域的实际路径不消费错误间距字段。相机中心射线采样地形，四角仍为局部平面投影，零高度字段和 Centerpoint 不作精确地面高程依据；SearchTaskAnalysis 在导出时重放事件，使用飞机位置最近邻地形计算 AGL。保持这些限制，扩大区域或修改算法须重验。T05 真实 G4 状态接入也已通过，T06 实体／姿态显示已通过，T07 航线／任务／区域已完成，下一卡 T08 为生命周期与恢复；T09／T10 完整显示联调和 T11 人工确认／发布尚待后续。

## G5-T03 地图与本地资源服务

T03 已完成，见 [地图报告](docs/g5-map-validation.md)。本地 PMTiles 经有界 Range／工作线程绘制；USGS 区域高度场直接使用 T02 椭球高，区域外明确为未验收参考椭球；Esri 按切换加载，失败恢复本地底图。字体与 Cesium 资源本地部署；T06 已选用 UCAV 完成转换。后续 T04 后端地形、T05 真实接入和 T06 实体／姿态显示已完成，下一卡为 T08。
...
T01 交付时只有工具环境指针 .tools/g5/current.json，页面为椭球与工程样片，候选 stageQualified=false；这一历史入口不代表当前地图能力。后续 T02 已完成，见 [地理检查报告](docs/g5-geography-validation.md)：原始 z10 异常保留历史；用户选定原矢量底图＋USGS 区域主地形，Copernicus 对照，WGS84／EGM96 标准化正高派生 AMASE DTED 与 Cesium 椭球高。完整矢量摘要／索引、区域来源／网格、独立数值及八组验收通过。T03 后续地图资格见上节，T04 后端地形资格也已通过，T05 状态接入也已通过，T06 实体显示也已通过，当前下一卡为 T08；正式前端仍未发布。源 NAD83／NAVD88 与 Copernicus EGM2008 按固定链转换，禁止重标 CRS、混合补值或用合成输出代替真实资格；全球地形不随区域自动合格。
...
实体 MSL／EGM96 改正一次到 WGS84 椭球高，未知基准／越界拒绝；姿态独立对照正式 AMASE Euler。显示时间为后端时间减 1 秒并限制于已收样本，暂停保持；10 分钟／2048 源样本，刷新重新积累。删除／恢复清理实体及跟随，完成保留实体。模型、定位／跟随／复位、快捷键、两模式真实时间和离线地图回归通过；beforeunload 必须先释放实体层再销毁地图 Viewer，后续修改保留此回归。下一卡 T08，不将本卡短程显示验收扩大为 T09／T10 全程或 T11 阶段资格。

T06 当前外观见 [金属材质预览](docs/g5-metal-display-preview.md)：银灰机体、GGX 高光与展示环境反射，原纹理和资产保持；阵营色细描边普通 1 像素、选中 1.5 像素，选中不换白边；标签／轨迹／列表继续同阵营色。固定屏幕尺寸和无远距点替代保持。实际 Affiliation 优先；当前 Unknown 的 400／500 按用户显示指定为蓝方、600 红方，详情标注来源，未知未指定保持黄，不猜测友敌、不修改后端。用户明确先完成配色，控制按后续任务实施；开始／暂停、倍速及只读时间进度条归 G6，进度条只显示当前仿真时间和进度，历史拖动回放仍归 G7。当前控制尚未接入，下一卡保持 G5-T08。
...
T07 已完成，见 [业务显示报告](docs/g5-missions-validation.md)。新增 apps/cesium_missions 和独立构建／验收／联合入口，严格组合合格 T06 项目副本，旧输入／候选／正式指针保持。19 项输入、普通／中文 407 个生产文件一致，1004 点数值和四组真实后端运行、独立区域／删除与离线资源回归通过。下一卡 T08；不提前登记 T09／T10 全程、T11 人工确认或阶段发布。

入口为 build-g5-missions.ps1（可加 ChinesePath）、tests/windows/g5-missions.tests.ps1 和 start-g5-missions-session.ps1；参数沿用 PythonExecutable，后两者要求 BuildRunId，Mode 支持 Gui／Headless。当前普通 g5-t07-build-20260921-210702-807，中文 g5-t07-build-20260921-210342-447，验收 g5-t07-test-20260921-210534-383；访问 8080，本次会话 request-stop 正常关闭本组。
...
日常入口改用 scripts/windows/start-g5-coverage-session.ps1（PythonExecutable、BuildRunId 必填，Mode Gui／Headless）；构建为 build-g5-coverage.ps1，验收为 tests/windows/g5-coverage.tests.ps1。全量覆盖父版本验收 g5-coverage-test-20260922-112501-163 保留；当前导航候选普通 g5-coverage-build-20260922-140926-685、中文 g5-coverage-build-20260922-140910-656，真实浏览器专项 g5-coverage-navigation-check-20260922-140940-708 通过，见报告第 7 节；入口复查 T07 及前置绑定。访问 8080，创建本次会话 request-stop 正常退出。新增 /api/coverage/v1/snapshot 只读路径，不改 G4 v1、不增加监听端口；旧来源／候选／正式指针保持。
...
当前用户操作要求为左拖平移、右拖环绕、滚轮缩放，左键点击仍选中对象。navigation.ts 用公开相机接口适配自由／跟随视角；跟随时左拖沿屏幕平面移动相机，不转动方向或解除跟随。松键／失焦／切换跟随结束平移，销毁释放监听；中键、触控及 Ctrl／Shift 映射保持。真实鼠标、模型选中、恢复自由视角及刷新均已核查。
