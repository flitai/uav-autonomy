...
import { MapNavigation } from './navigation.js';
import './style.css';
...
export class CoveragePanel {
  ...
  private readonly cameraHint=node('p','左键拖动：平移；右键拖动：环绕。滚轮缩放；左键点击选择目标，中键转动视角。');
  private readonly navigation:MapNavigation;
  ...
  constructor(readonly viewer:Viewer,readonly connection:ReadOnlyConnection,readonly entities:EntityLayer,readonly heights:Heights){
    this.navigation=new MapNavigation(viewer);
